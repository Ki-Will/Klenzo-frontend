...
async function apiFetch<T>(
  ...
  try {
    ...
    if (res.status === 401 && retry) {
      // Don't attempt refresh for the refresh endpoint itself — avoids infinite loops.
      // Also skip for login/register since those don't use session cookies.
      const skipRefresh =
      ...
      if (!skipRefresh) {
        ...
        // Refresh failed — let the caller (AuthProvider/page) handle redirect
