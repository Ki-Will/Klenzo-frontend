  ...
  return (
    <main className="px-6 lg:px-12 py-6 min-h-screen">
      <div className="max-w-3xl mx-auto space-y-8">
        {/* Back */}
        <Link href="/expenses" className="flex items-center gap-2 text-secondary-text hover:text-primary transition-colors text-sm">
          <span className="material-symbols-outlined text-sm">arrow_back</span>
          Back to Expenses
        </Link>

        {/* Hero */}
        <div className="bg-card rounded-2xl p-8 flex flex-col items-center text-center space-y-4">
          <div className="w-20 h-20 rounded-full bg-card-highest flex items-center justify-center text-primary">
            <span className="material-symbols-outlined text-4xl" style={{ fontVariationSettings: "'FILL' 1" }}>
              {txIcon(transaction.category, transaction.transactionType)}
            </span>
          </div>
          <div>
            <h1 className="text-3xl font-headline font-bold text-primary-text">
              {transaction.description || transaction.category || "Transaction"}
            </h1>
            <p className="text-secondary-text text-sm uppercase tracking-wider mt-1">
        ...
        <div className="bg-card rounded-2xl p-8 space-y-6">
          ...
          {[
            ...
            <div key={item.label} className="flex justify-between items-center">
              <span className="text-secondary-text text-sm">{item.label}</span>
              <span className="text-primary-text font-medium text-sm capitalize">{item.value}</span>
            </div>
          ))}
        </div>

        {/* AI Insight */}
        <div className="bg-card-deep border border-outline/10 rounded-2xl p-8">
        ...
        <div className="flex gap-4">
          <Link href={`/expenses/add`} className="flex-1 py-4 bg-card-high hover:bg-card-highest rounded-full font-bold text-sm transition-all flex items-center justify-center gap-2">
            <span className="material-symbols-outlined text-sm">edit</span>
            Edit
          </Link>
          <button onClick={handleDelete} disabled={deleting}
            className="flex-1 py-4 bg-error/10 hover:bg-error/20 text-error rounded-full font-bold text-sm transition-all flex items-center justify-center gap-2 disabled:opacity-50">
