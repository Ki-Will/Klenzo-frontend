  ...
  return (
    <main className="px-6 lg:px-12 py-6 min-h-screen">
      <div className="max-w-3xl mx-auto space-y-8">
        <Link href={`/groups/${id}`} className="flex items-center gap-2 text-secondary-text hover:text-primary transition-colors text-sm">
          <span className="material-symbols-outlined text-sm">arrow_back</span>
          Back to {group.name}
        </Link>

        <div>
          <h1 className="text-4xl font-headline font-extrabold tracking-tighter text-primary-text">Balance Overview</h1>
          <p className="text-secondary-text mt-2">{group.name} • Settlement Summary</p>
        </div>

        {/* My net balance */}
        {myBalance && (
          <div className="bg-card rounded-2xl p-8 text-center">
            <p className="text-secondary-text text-sm uppercase tracking-widest mb-4">
              {myBalance.balance < 0 ? "You owe in total" : myBalance.balance > 0 ? "You are owed" : "You are settled up"}
            </p>
            <p className={`text-6xl font-headline font-extrabold tracking-tighter ${myBalance.balance < 0 ? "text-error" : myBalance.balance > 0 ? "text-primary" : "text-secondary-text"}`}>
              ${totalOwed.toFixed(2)}
            </p>
          </div>
        )}
          ...
          <div className="bg-card rounded-2xl p-8 space-y-4">
            ...
            {debtors.map((debtor) =>
              creditors.map((creditor) => {
                ...
                return (
                  <div key={`${debtor.userId}-${creditor.userId}`} className="flex items-center gap-4 p-4 bg-card-deep rounded-2xl">
                    <div className="w-10 h-10 rounded-full bg-card-highest flex items-center justify-center text-xs font-bold text-secondary-text">
                      {debtor.name.slice(0, 2).toUpperCase()}
                    </div>
                    <div className="flex-grow flex items-center gap-2 min-w-0">
                      <span className="text-sm font-medium text-primary-text truncate">{debtor.name}</span>
                      <span className="material-symbols-outlined text-secondary-text text-sm flex-shrink-0">arrow_forward</span>
                      <span className="text-sm font-medium text-primary-text truncate">{creditor.name}</span>
                    </div>
                    <span className="font-bold text-error flex-shrink-0">${amount.toFixed(2)}</span>
                    {isMe && (
                      <button
                        onClick={() => handleSettle(creditor.userId, amount)}
                        disabled={settling}
                        className="px-4 py-2 bg-primary text-on-primary rounded-full text-xs font-bold hover:brightness-110 active:scale-95 transition-all disabled:opacity-50 flex-shrink-0 cursor-pointer"
                      >
                        {settling ? "…" : "Settle"}
                      </button>
                    )}
        ...
        {balances.length > 0 && (
          <div className="bg-card rounded-2xl p-8 space-y-3">
            ...
            {balances.map((b) => (
              <div key={b.userId} className="flex items-center justify-between p-4 bg-card-deep rounded-2xl">
                ...
                <span className={`font-bold text-sm ${b.balance < 0 ? "text-error" : b.balance > 0 ? "text-primary" : "text-secondary-text"}`}>
